...
export const authAPI = {
  ...
  forgotPassword(forgotPasswordData: ForgotPasswordDataType) {
    return axios.post<ForgotPasswordDataType, AxiosResponse<{ info?: string; response?: { data: { error: string } } }>>("https://neko-back.herokuapp.com/2.0/auth/forgot", forgotPasswordData, {
      withCredentials: true,
    })
  },
  setNewPassword(setNewPasswordData: SetNewPasswordDataType) {
    return axios.post<SetNewPasswordDataType, AxiosResponse<{ info?: string; response?: { data: { error: string } } }>>(
